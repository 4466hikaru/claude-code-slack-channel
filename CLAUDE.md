...
- `scripts/pickup-to-execute.ts` (+ `scripts/pickup-to-execute.test.ts`) — executor-side CLI for `handoff/to-execute/*.md` assignments. `wait --poll-ms 5000` is the execution-role resting state: wait for one assignment, atomically claim it, print the body, then exit so Claude can perform the task and write `handoff/from-execute/done-*.md`. Runbook: [`docs/executor-pickup-runbook.md`](docs/executor-pickup-runbook.md).
- `scripts/terminal-consult.ts` (+ `scripts/terminal-consult.test.ts`) / `scripts/g2-consult.sh` — G2 / terminal input を Slack DM + `codex-consult-queue` に登録する consult relay。queue 登録のみで実行・merge はしない。Runbook: [`docs/terminal-consult-relay.md`](docs/terminal-consult-relay.md).
