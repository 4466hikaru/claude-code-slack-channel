...
- `scripts/terminal-consult.ts` (+ `scripts/terminal-consult.test.ts`) — G2 / terminal input を Slack DM + `codex-consult-queue` に登録する consult relay。queue 登録のみで実行・merge はしない。Runbook: [`docs/terminal-consult-relay.md`](docs/terminal-consult-relay.md).
