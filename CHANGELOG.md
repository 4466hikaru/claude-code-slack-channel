...
- **Terminal consult relay for G2 / terminal workflows.** Adds scripts/terminal-consult.ts plus the shorter scripts/g2-consult.sh wrapper, turning terminal text into a Slack DM root message plus a durable codex-consult-queue request, reusing the existing Codex plan / Hikaru approve / executor dispatch path. The relay only queues consults, never executes work, and redacts token-like text before writing to Slack or handoff storage. Includes unit coverage and docs/terminal-consult-relay.md.
