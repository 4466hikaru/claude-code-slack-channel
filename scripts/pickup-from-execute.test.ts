...
import {
  ...
  MIN_WAIT_POLL_MS,
  parseWaitOptions,
} from './pickup-from-execute'

describe('parseWaitOptions (pickup-from-execute)', () => {
  ...
  test('rejects invalid poll intervals', () => {
    expect(() => parseWaitOptions(['--poll-ms', String(MIN_WAIT_POLL_MS - 1)])).toThrow('between')
    expect(() => parseWaitOptions(['--poll-ms', String(MAX_WAIT_POLL_MS + 1)])).toThrow('between')
