 ...
 *   wait                       Wait until one assignment appears, claim it, then print
 *   help                       Usage
 *
 * The CLI is discovery + claim only. The actual implementation of an
 * assignment is the executor session's responsibility: read the body
 * printed by show / claim / wait, do the work, write a done-*.md per
 * the existing relay format.
 *
 * Safety:
 *   - If handoff/abort-lv2 is present, all subcommands exit non-zero
 *     with a clear abort message. The CLI never starts work under abort.
 *   - claim / wait use renameSync for atomic single-consumer semantics.
 *   - show and list are read-only.
 *   - wait only waits + claims one assignment; it does not execute the
 *     assignment body or run as a persistent daemon.
 *   - No Slack API call, no DB, no destructive delete outside the claim
 *     move, which is a relocation rather than a delete.
 */
...
const USAGE = [
  ...
  '  bun scripts/pickup-to-execute.ts wait [--poll-ms <ms>] [--timeout-ms <ms>]',
...
function printEntryDetails(entry: AssignmentEntry, pathLabel: string): void {
  process.stdout.write(pathLabel + '\n# correlation_id: ' + entry.correlation_id + '\n')
  if (entry.risk_level) process.stdout.write('# risk_level: ' + entry.risk_level + '\n')
  if (entry.dev_verification) process.stdout.write('# dev_verification: ' + entry.dev_verification + '\n')
  ...
  if (entry.codex_plan_ref) process.stdout.write('# codex_plan_ref: ' + entry.codex_plan_ref + '\n')
}

function printClaimedAssignment(entry: AssignmentEntry, dest: string): void {
  ...
  process.stdout.write(
    '# next: write the result to handoff/from-execute/' +
      suggestedDone +
      '\n' +
      '#       with frontmatter type: "done", done_id: "' +
      doneId +
      '",\n' +
      '#       status: "complete"|"blocked"|"failed", summary: <one line>\n' +
      '#       needs_review: "true"|"false", related_bd: "' +
      (entry.related_task ?? '') +
      '".\n',
  )
...
function claimAndPrint(entry: AssignmentEntry): void {
  let dest: string
  try {
  ...
  printClaimedAssignment(entry, dest)
}
...
function cmdShow(identifier: string | undefined): void {
  ...
  if (found.kind === 'none') {
    exitErr('[pickup] no pending assignment matches "' + identifier + '" (run list to see candidates)')
  }
...
async function cmdWait(args: string[]): Promise<void> {
  ...
  try {
    ...
    exitErr('[pickup] ' + (e instanceof Error ? e.message : String(e)) + '\n' + USAGE)
  }

  const startedAt = Date.now()
  process.stderr.write(
    '[pickup] waiting for assignments under ' +
      TO_EXECUTE_DIR +
      ' (poll=' +
      opts.pollMs +
      'ms timeout=' +
  ...
  for (;;) {
    ...
    if (opts.timeoutMs !== null) {
      ...
      if (elapsed >= opts.timeoutMs) {
        process.stdout.write('(no pending assignments under ' + TO_EXECUTE_DIR + ' before timeout)\n')
        return
      }
...
if (import.meta.main) {
  main().catch((e) => {
    exitErr('[pickup] unexpected error: ' + (e instanceof Error ? e.message : String(e)))
  })
}
